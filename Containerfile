...
RUN cd /tmp && mkdir -p make && tar -xf make.tar.gz -C make --strip-components 1 && cd make && CC=tcc LD=tcc ./configure --host=x86_64-unknown-linux-musl --build=x86_64-unknown-linux-musl --disable-dependency-tracking && ./build.sh && ./make install && cd .. && rm -rf make make.tar.gz
